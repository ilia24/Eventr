class EventsController < ApplicationController
  before_action :ensure_logged_in, only: [:new, :create, :destroy, :edit, :update, :join]
  before_action :ensure_hostinfo_filled_out, only: [:new, :create]
  before_action :ensure_eventrinfo_filled_out, only: [:join]
...
private
  def event_params
    params.require(:event).permit(:name, :price, :picurl, :picurl2, :picurl3, :cover, :location, :date, :event_style, :time, :description)
  end
